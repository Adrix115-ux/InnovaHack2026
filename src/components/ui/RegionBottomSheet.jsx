...
// Aquí mapeamos los IDs que manda el SVG interactivo con sus datos visuales
const REGION_DATA = {
  'santa_cruz': {
    label: 'Santa Cruz',
    emoji: '🌴',
...
/**
 * Props:
 * - region (string|null): ID de la región activa (ej: "santa_cruz")
...
export default function RegionBottomSheet({ region, products, onClose }) {
  ...
  // Extraer metadatos de la zona (si no existe en el diccionario, usa valores por defecto)
  const zoneData = REGION_DATA[region];
  const regionLabel = zoneData?.label ?? (region ? region.replace('_', ' ') : 'Región');
  const regionEmoji = zoneData?.emoji ?? '📍';
  const regionColor = zoneData?.color ?? '#4ade80';
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-3 shrink-0 border-b border-green-900/30">
            ...
            >
              ...
              <div className="leading-tight">
                ...
                <h2
                    className="text-lg font-black leading-none capitalize"
